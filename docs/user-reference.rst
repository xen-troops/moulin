...
The :code:`--dep` command line option is internal and hidden from
:code:`-h` output. It is used by `moulin` to generate dynamic dependency
...
the layer list defined in the Moulin YAML manifest. Similarly,
:code:`--utility-builders-custom_script` is used by the
:code:`custom_script` builder to run embedded :code:`inline_script`
content.
...
* :code:`dependency_policy` - selects which dynamic dependencies are
  ...
  * :code:`fetched_files` - use only files reported by fetchers. This
    matches the dependency set generated by older Moulin versions through
    :code:`--fetcherdep`.
...
The custom-script builder runs a user-provided script. Use it for custom build,
packaging, signing, or image assembly steps that are not covered by a dedicated
builder.

A custom script can come from either:

* :code:`script` - path to an existing script or executable.
* :code:`inline_script` - script content embedded into the Moulin
  configuration. Moulin writes it to a temporary executable file before running
  it.

Moulin always writes the whole builder node to a generated configuration file
under :code:`work_dir`. The custom script receives that file path as the last
command line argument. The generated file contains the complete builder
configuration, including :code:`script` or :code:`inline_script`,
:code:`args`, :code:`config`, and any custom fields added by the project:

.. code-block:: text

  <script> <args> <generated-config-file>
...
.. code-block:: yaml
  ...
  builder:
    ...
    inline_script: |
      ...
      with tarfile.open("release.tar.gz", "w:gz") as archive:
          archive.add("boot.img")
          archive.add("rootfs.img")
    target_images:
      - "release.tar.gz"
    additional_deps:
      - "boot.img"
...
Moulin stores :code:`inline_script` in a temporary executable file and runs it
directly. Use a shebang, such as :code:`#!/usr/bin/env python3`, to select the
...
* :code:`script` or :code:`inline_script` - custom script to run. Exactly one of
  these options must be present.

  * :code:`script` is a path to an external script which performs custom
    actions.
  * :code:`inline_script` is embedded script content. Moulin writes it to a
    temporary executable file before running it.

  Moulin runs the script path directly. Generated :code:`inline_script` files
  are made executable by Moulin; external :code:`script` files must already be
  executable. Direct execution requires a shebang or another host-supported
  executable format.
...
* :code:`args` - additional arguments passed to the script before the generated
  configuration file. Can be passed as a string or a list.
...
* Custom builder-specific fields can be added to this section. Moulin preserves
  them in the generated configuration file for the script to read.
